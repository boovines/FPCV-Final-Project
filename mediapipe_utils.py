...
class HandDetector:
    def __init__(self):
        """Initialize MediaPipe Hand Landmarker for real-time hand detection."""
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[np.ndarray, List[dict]]:
        ...
        Returns:
            ...
            hand_data_list contains dict with 'landmarks', 'handedness' for each detected hand
        """
        ...
        if results.multi_hand_landmarks:
            for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                # Draw additional knuckle and joint markers for better visibility
    ...
    def get_finger_tips(self, hand_data: List[dict]) -> dict:
        ...
        Args:
            hand_data: List of hand data from process_frame
            
        Returns:
            Dict with 'left' and 'right' keys, each containing 'thumb' and 'index' coordinates
        """
    ...
    def _draw_enhanced_landmarks(self, frame: np.ndarray, hand_landmarks, frame_shape: tuple):
        """
        Draw enhanced landmarks with larger, more visible markers for knuckles and joints.
        ...
        finger_mcps = [2, 5, 9, 13, 17]   # Metacarpophalangeal joints (knuckles)
        
        # Draw finger tips with large circles
        for idx in finger_tips:
            landmark = hand_landmarks.landmark[idx]
            x = int(landmark.x * w)
            y = int(landmark.y * h)
            cv2.circle(frame, (x, y), 8, (0, 255, 0), -1)  # Green filled circles
            cv2.circle(frame, (x, y), 10, (255, 255, 255), 2)  # White border
        
        # Draw PIP joints with medium circles
        for idx in finger_pips:
            landmark = hand_landmarks.landmark[idx]
            x = int(landmark.x * w)
            y = int(landmark.y * h)
            cv2.circle(frame, (x, y), 6, (0, 255, 255), -1)  # Yellow filled circles
            cv2.circle(frame, (x, y), 8, (0, 0, 0), 2)  # Black border
        
        # Draw MCP joints (knuckles) with large squares
        for idx in finger_mcps:
            landmark = hand_landmarks.landmark[idx]
            x = int(landmark.x * w)
            y = int(landmark.y * h)
            cv2.rectangle(frame, (x-6, y-6), (x+6, y+6), (255, 0, 0), -1)  # Red filled squares
            cv2.rectangle(frame, (x-8, y-8), (x+8, y+8), (255, 255, 255), 2)  # White border
        
        # Draw wrist with special marker
        wrist = hand_landmarks.landmark[0]
        x = int(wrist.x * w)
        y = int(wrist.y * h)
        cv2.circle(frame, (x, y), 10, (255, 0, 255), -1)  # Magenta filled circle
        ...
        # Label thumb and index tips (most important for frame detection)
